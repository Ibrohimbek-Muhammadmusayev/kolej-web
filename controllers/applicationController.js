...
exports.createApplication = async (req, res) => {
    try {
        ...
        // Notify bot subscribers
        let notificationMsg = `🔔 Yangi xabar/ariza qabul qilindi!\n\n`;
        if (appData.fullName) notificationMsg += `👤 F.I.SH: ${appData.fullName}\n`;
        if (appData.phone) notificationMsg += `📞 Telefon: ${appData.phone}\n`;
        if (appData.message) notificationMsg += `✉️ Xabar: ${appData.message}\n`;
        if (appData.education) notificationMsg += `🏫 Tamomlagan: ${appData.education}\n`;

        botManager.sendNotification(notificationMsg).catch(err => {
            console.error("Failed to send app notification via bot", err);
        });
