...
async function loadStudents() {
    ...
    try {
        ...
        tbody.innerHTML = allStudents.map(item => `
            <tr class="hover:bg-gray-50 transition-colors border-b last:border-0">
                ...
                <td class="p-4 text-right space-x-2">
                    <button onclick="editStudent(${item.id})" class="text-blue-600 hover:bg-blue-100 p-2 rounded transition-colors" title="Tahrirlash">✏️</button>
                    <button onclick="deleteStudent(${item.id})" class="text-red-600 hover:bg-red-100 p-2 rounded transition-colors" title="O'chirish">🗑️</button>
                </td>
            </tr>
        `).join('');
...
async function handleSave(e) {
    ...
    try {
        ...
        if (res && res.ok) {
            admin.showGlobalSuccess(currentId ? "O'zgarishlar saqlandi" : "Yangi o'quvchi qo'shildi");
            closeModal();
            loadStudents();
        }
    } catch (err) {
        console.error(err);
        admin.showGlobalError("Saqlashda xatolik yuz berdi");
    }
}
