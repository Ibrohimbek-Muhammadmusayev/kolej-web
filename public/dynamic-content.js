...
class CMS {
    ...
    updateStaticContent() {
        ...
        // Hero Title
        const heroTitles = document.querySelectorAll('[data-i18n="hero-title"]');
        heroTitles.forEach(el => {
            el.textContent = this.get(this.settings, 'hero_title');
        });
        ...
        // Map
        const mapIframes = document.querySelectorAll('.map-container iframe, #contact iframe');
        mapIframes.forEach(iframe => {
            if (this.settings.map_url) iframe.src = this.settings.map_url;
        });

        // Update the translations dictionary dynamically from settings
        if (window.translations) {
            ['uz', 'ru', 'en'].forEach(lang => {
                if (window.translations[lang]) {
                    if (this.settings[`about_desc_${lang}`]) {
                        window.translations[lang]['about-hero-desc'] = this.settings[`about_desc_${lang}`];
                    }
                    if (this.settings[`about_history_${lang}`]) {
                        window.translations[lang]['about-history-text'] = this.settings[`about_history_${lang}`];
                    }
                    if (this.settings[`about_mission_${lang}`]) {
                        window.translations[lang]['about-mission-text'] = this.settings[`about_mission_${lang}`];
                    }
                }
            });
        }

        // Apply immediately to elements if present
        const aboutDesc = document.querySelector('[data-i18n="about-hero-desc"]');
        if (aboutDesc) {
            const val = this.get(this.settings, 'about_desc');
            if (val) aboutDesc.textContent = val;
        }

        const aboutHistory = document.querySelector('[data-i18n="about-history-text"]');
        if (aboutHistory) {
            const val = this.get(this.settings, 'about_history');
            if (val) aboutHistory.textContent = val;
        }

        const aboutMission = document.querySelector('[data-i18n="about-mission-text"]');
        if (aboutMission) {
            const val = this.get(this.settings, 'about_mission');
            if (val) aboutMission.textContent = val;
        }
    }
    ...
    setupContactForm() {
        ...
        if (form) {
            form.addEventListener('submit', async (e) => {
                ...
                const originalBtnText = submitBtn ? submitBtn.innerHTML : 'Yuborish';
